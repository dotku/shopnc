...
class taobao_importControl extends BaseSellerControl {
	private function checkStore(){
        if(!checkPlatformStore()){
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {showMessage(L('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . L('store_goods_index_goods_limit1'), 'index.php?act=store_goods&op=goods_list', 'html', 'error');
                }
            }
        }
    }
	...
	public function indexOp(){
		...
		if(!$_POST){
			 ...
			 * 获取商品分类
			 */
			$gc	= Model('goods_class');$gc_list	= $gc->getGoodsClassList(array('gc_parent_id'=>'0'));
			...
			if($_GET['step'] != ''){Tpl::output('step',$_GET['step']);}else{Tpl::output('step','1');}
		}else{
			$file	= $_FILES['csv'];
			
			if(empty($file['name'])){showMessage($lang['store_goods_import_choose_file'],'','html','error');}
			...
			if(!is_array($gc_row) or count($gc_row) == 0){showMessage($lang['store_goods_import_wrong_class1'],'','html','error');
			}
